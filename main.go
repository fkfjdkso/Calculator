...
const (
	outOfRange             = "Введенное число не принадлежит отрезку от 1 до 10"
	nonIntInput            = "Введенное число(числа) не является(являются) целым(и)"
	romanianNegativeOrZero = "Римская система счисления не имеет числа 0 и отрицательных чисел"
	numberSystemMixed      = "Используются разные системы счисления"
	tooManyOrWrongActions  = "Формат математической операции не удовлетворяет заданию или не является математической операцией"
)
